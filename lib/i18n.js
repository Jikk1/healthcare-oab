...
export function setLocale(lang) {
  ...
  // Страницы с JS-рендерами (t()) слушают это событие и перерисовывают контент.
  document.dispatchEvent(new CustomEvent('hc:langchange', { detail: { lang: l } }));
}
...
/** Строка из JS-кода: RU — источник (второй аргумент), EN — по ключу словаря.
 *  Возвращает язык, актуальный в момент рендера; уже отрендеренный динамический
 *  контент перерисовывается страницей при следующем рендере. */
export function t(key, ru) {
  return getLocale() === 'en' ? (EN[key] ?? ru) : ru;
}

/** Повторно применяет текущий язык к data-i18n-узлам. Вызывать после
 *  динамического рендера разметки с data-i18n (форма, собранная из JS). */
