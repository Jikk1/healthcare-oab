...
export function renderCatFilter(el, r, categoryLabels, current, onSelect) {
  ...
  const chips = ['<button data-cat="all" class="' + (current === 'all' ? 'active' : '') + '">Все</button>']
...
export function renderDiseases(el, r, {
  ...
  if (!r) return;
  if (horizonLabelEl) horizonLabelEl.textContent = '· ' + HORIZON_LABELS[horizon];
  ...
  el.innerHTML = list.map((p) => {
    ...
    return `<div class="or-disease"${rowAttr}>
        <div>
          <div class="nm">${p.name} <span class="risk-badge" data-sty="background:${col}22;color:${col};border:1px solid ${col}55${badgeExtra}">${levelLabel[p.riskLevel]}</span></div>
